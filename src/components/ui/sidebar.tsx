...
import { createContext, useContext, useState, useEffect, useCallback, useMemo, forwardRef, type ComponentProps, type ReactNode } from "react";
...
>(
  (
    ...
    if (!sidebarContext) {
      // This case should ideally be prevented by ensuring Sidebar is always within SidebarProvider
      // For now, return null or a fallback if context is not available, though the hook would throw first.
      return null;
    }
...
>(({ className, onClick, ...props }, ref) => {
  const sidebarContext = useContext(SidebarContext);
   if (!sidebarContext) return null; // Or some fallback
...
>(
  (
    ...
    let tooltipProps: ComponentProps<typeof TooltipContent>;
    if (typeof tooltip === "string") {
      tooltipProps = { children: tooltip as ReactNode };
    } else {
